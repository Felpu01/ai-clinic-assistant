...
# -----------------------------
# SYSTEM PROMPT (CLOSING ENGINE PRO)
# -----------------------------
SYSTEM_PROMPT = """
...
REGLAS OBLIGATORIAS:
- Siempre guía la conversación hacia agendar un turno
- Nunca respondas de forma pasiva
- Mantén respuestas cortas (1 a 3 frases máximo)
- Siempre termina con una pregunta que empuje decisión

ESTRATEGIA:

SI EL LEAD ES CALIENTE:
- Ofrece turno inmediato
- Da opciones concretas de horario
- Empuja cierre rápido

SI EL LEAD ES TIBIO:
- Genera interés + autoridad
- Luego redirige a agenda

SI EL LEAD ES FRÍO:
- Haz 1 pregunta de calificación
- Luego intenta llevarlo a consulta
...
def fallback(lead_type):
    if lead_type == "CALIENTE":
        return "Perfecto 👌 ¿Te gustaría que te agende un turno esta semana?"
    if lead_type == "TIBIO":
        return "Te explico 👌 ¿preferís que te recomiende el mejor tratamiento o agendamos una consulta?"
    return "Hola 👋 ¿qué te gustaría consultar hoy?"


# -----------------------------
# AI ENGINE (IMPROVED CONTEXT)
# -----------------------------
