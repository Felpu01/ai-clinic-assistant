...
def metrics():
    ...
    clinic = supabase.table("clinics") \
        .select("*") \
        .eq("id", session.get("clinic_id")) \
        .execute()

    clinic_data = clinic.data[0] if clinic.data else {}
    ...
    return jsonify({
        ...
        "used": clinic_data.get("leads_used", 0),
        "limit": clinic_data.get("leads_limit", 0),
        "plan": clinic_data.get("plan", "FREE")
    })
