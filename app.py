...
# -----------------------------
# LEAD SCORE SYSTEM (NUEVO)
# -----------------------------
def calculate_score(lead_type):
...
def chat():
    try:
        ...
        score = calculate_score(lead_type)
        stage = pipeline_stage(score)
        ...
        LEADS[user_id]["score"] = score
        LEADS[user_id]["stage"] = stage
...
# -----------------------------
# METRICS (NUEVO - CLAVE SaaS)
# -----------------------------
@app.route("/metrics", methods=["GET"])
def metrics():
    total = len(LEADS)
    caliente = sum(1 for l in LEADS.values() if l["score"] >= 80)
    tibio = sum(1 for l in LEADS.values() if 50 <= l["score"] < 80)
    frio = sum(1 for l in LEADS.values() if l["score"] < 50)
...
@app.route("/")
def home():
    return "AI Clinic CRM V2 Stable 🚀"
